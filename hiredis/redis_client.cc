...
RedisClient::RedisClient(const std::string hostname, const int port)
    : hostname_(hostname),
      port_(port),
      context_(nullptr, redisAsyncFree),
...
int RedisClient::subscribe(const std::string channel, subscribe_callback cb) {
    ...
    cb_ = cb;
    redisAsyncCommand(context_.get(), redis_async_callback, this, "SUBSCRIBE %s", channel.c_str());
    return REDIS_OK;
}
...
int main(int argc, char **argv) {
    ...
    client.subscribe("test", std::bind(&Config::update, &config, std::placeholders::_1));
    client.start();
    while(1) {
        ;
    }
